...
import '../models/live_stream_reaction_model.dart';
...
class FirebaseRemoteDatasourceImpl implements FirebaseRemoteDataSource {
  ...
  final _liveStreamReactionsCollection = FirebaseFirestore.instance.collection(
    "liveStreamReactions",
  );
  @override
  Stream<List<TextMessageEntity>> getTextMessages() {
  ...
  @override
  Stream<List<LiveStreamReactionEntity>> getLiveStreamReactions() {
    return _liveStreamReactionsCollection.snapshots().map(
      (querySnapshot) => querySnapshot.docs
          .map(
            (docSnapshot) => LiveStreamReactionModel.fromSnapshot(docSnapshot),
          )
          .toList(),
    );
  }
